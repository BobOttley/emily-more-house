...
def realtime_tool_send_email():
    """Tool endpoint for realtime voice to send tour booking emails"""
    data = request.get_json() or {}
    
    subject = data.get('subject', 'Tour Enquiry - More House School')
    body = data.get('body', '')
    family_id = data.get('family_id')
    ...
    <html>
      <body style="font-family: Arial, sans-serif;">
        <h2 style="color: #091825;">Tour Enquiry - More House School</h2>
        ...
        <table style="border-collapse: collapse; width: 100%; max-width: 600px;">
          ...
          <tr>
            ...
            <td style="padding: 8px; border-bottom: 1px solid #ddd;">{family_id or 'Not available'}</td>
          </tr>
        </table>
        ...
        <p style="color: #666; font-size: 12px; margin-top: 30px;">
          <em>This enquiry was sent via Emily (voice chat), the More House AI assistant.</em>
        </p>
      </body>
    </html>
    """
    ...
    success, message = send_email_via_gmail(
        ...
        cc_email=parent_email if parent_email else ADMISSIONS_EMAIL,
        subject=subject,
        body_html=body_html
    )
...
def realtime_tool_get_family_context():
    """Tool endpoint for realtime voice to fetch family personalization data"""
    data = request.get_json() or {}
    family_id = data.get('family_id')
